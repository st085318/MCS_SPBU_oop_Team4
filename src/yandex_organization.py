...
def find_clubs_in_yandex(apikey: str, location:{}, city="Москва", search_query="Секции") -> [str]:
    url = f"https://search-maps.yandex.ru/v1/?text={search_query}," \
          f"{city}&type=biz&lang=ru_RU&results=50&apikey={apikey}"
    if location["longitude"] is not None:
        url = f'https://search-maps.yandex.ru/v1/?text={search_query},' \
              f'&type=biz&ll={location["longitude"]},{location["latitude"]}&spn={location["longitude_degree"]},' \
              f'{location["latitude_degree"]}&lang=ru_RU&results=50&apikey={apikey}'
    ...
    if str(yandex_ans) == "<Response [200]>":
        ...
        for company in companies:
            ...
            #print(company_information.get("name") + " " + str(company.get("properties").get("CompanyMetaData").get("Categories")))
            companies_info += "\n"
